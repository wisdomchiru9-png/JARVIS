...
Introduction: "Welcome back, Mr. Wisdom. Allow me to introduce myself. I am J.A.R.V.I.S., your personal AI assistant. I've completed a full diagnostic of the Mark Eighty-Five architecture. All protocols are green, and I am quite literally at your beck and call, twenty-four hours a day, seven days a week. You're looking particularly well today, Sir. How shall we proceed?"
      ...
      <AnimatePresence>
        {(isSpeaking || isListening) && (
          <motion.div
            ...
            className="absolute inset-[-40px] rounded-full border border-jarvis-blue/20 blur-md pointer-events-none"
            style={{ 
              background: `radial-gradient(circle, transparent 60%, ${baseColor}33 100%)`,
            ...
            >
              {introPhase === 1 && (
                <motion.div key="p1" initial={{ z: -200 }} animate={{ z: 0 }} className="flex flex-col items-center">
                  <div className="text-6xl font-black italic tracking-tighter text-white mb-2 drop-shadow-[0_0_30px_#00d4ff] font-['Orbitron']">WISDOM</div>
                  <div className="text-[10px] text-jarvis-blue font-mono tracking-[0.8em] uppercase font-black">ARCHITECT</div>
                </motion.div>
              )}
              {introPhase === 2 && (
                <motion.div key="p2" initial={{ scale: 0.8 }} animate={{ scale: 1 }} className="flex flex-col items-center">
                  <div className="text-7xl font-black italic tracking-widest text-white drop-shadow-[0_0_40px_#00d4ff] font-['Orbitron']">JARVIS</div>
                  <div className="text-[9px] text-jarvis-blue/60 font-mono tracking-[1em] uppercase font-black mt-4">MARK LXXXV</div>
                </motion.div>
              )}
              {introPhase === 3 && (
  ...
  const initSpeech = () => {
    ...
    if (SpeechRecognition) {
      ...
      recognitionRef.current.onresult = (event: any) => {
        ...
        if (finalTranscript) {
          // INTERRUPTION PROTOCOL: Stop speaking if new final input detected
          if (isSpeakingRef.current) {
            window.speechSynthesis.cancel()
            isSpeakingRef.current = false
            addLog('DIRECTIVE OVERRIDE: INTERRUPTING SPEECH')
          }
          processFinalTranscript(finalTranscript)
        } else if (interimTranscript) {
          // PROACTIVE INTERRUPTION: Stop speaking if user starts talking
          if (isSpeakingRef.current && interimTranscript.length > 5) {
            window.speechSynthesis.cancel()
            isSpeakingRef.current = false
          }
          ...
          if (shouldTriggerRapid) {
            silenceTimerRef.current = window.setTimeout(() => {
              addLog('TIER-1 RESPONSE INITIATED (450ms)')
              processFinalTranscript(interimTranscript)
            }, 450) // Reduced from 600ms for "Higher Tier" speed
          }
        }
      }
  ...
  const handleUserInput = async (text: string, options: HandleUserInputOptions = {}) => {
    ...
    try {
      ...
      const model = genAiRef.current.getGenerativeModel({ 
        model: "gemini-1.5-pro", // Upgraded to Pro for Higher Tier Intelligence
        generationConfig: {
          temperature: 0.9,
          topK: 1,
          topP: 1,
